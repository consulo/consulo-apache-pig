...
public class PigParser implements PsiParser {
  ...
  @NotNull
  public ASTNode parse(IElementType root_, PsiBuilder builder_, LanguageVersion languageVersion_) {
    int level_ = 0;
    boolean result_;
    ...
    else if (root_ == PIG_RMF_CLAUSE) {
      result_ = rmf_clause(builder_, level_ + 1);
    }
  ...
  private static boolean cond_1_0(PsiBuilder builder_, int level_) {
    ...
    result_ = result_ && not_cond(builder_, level_ + 1);
    if (!result_) {
      marker_.rollbackTo();
    }
    else {
  ...
  public static boolean op_clause(PsiBuilder builder_, int level_) {
    ...
    if (!result_) result_ = rmf_clause(builder_, level_ + 1);
    if (result_) {
  ...
  public static boolean register_clause(PsiBuilder builder_, int level_) {
    ...
    Marker marker_ = builder_.mark();
    enterErrorRecordingSection(builder_, level_, _SECTION_GENERAL_, null);
    result_ = consumeTokens(builder_, 1, PIG_REGISTER, PIG_FILENAME);
    pinned_ = result_; // pin = 1
    if (result_ || pinned_) {
  ...
  /* ********************************************************** */
  // RMF FILENAME
  public static boolean rmf_clause(PsiBuilder builder_, int level_) {
    if (!recursion_guard_(builder_, level_, "rmf_clause")) return false;
    if (!nextTokenIs(builder_, PIG_RMF)) return false;
    boolean result_ = false;
    Marker marker_ = builder_.mark();
    result_ = consumeTokens(builder_, 0, PIG_RMF, PIG_FILENAME);
    if (result_) {
      marker_.done(PIG_RMF_CLAUSE);
    }
    else {
      marker_.rollbackTo();
    }
  ...
  private static boolean unary_cond_1(PsiBuilder builder_, int level_) {
    ...
    unary_cond_1_0(builder_, level_ + 1);
    return true;
  }
  ...
  private static boolean unary_cond_1_0(PsiBuilder builder_, int level_) {
    ...
    boolean result_ = false;
    Marker marker_ = builder_.mark();
    result_ = unary_cond_1_0_0(builder_, level_ + 1);
    if (!result_) result_ = unary_cond_1_0_1(builder_, level_ + 1);
    if (!result_) {
      marker_.rollbackTo();
    }
    else {
  ...
  // IS (NOT)? NULL
  private static boolean unary_cond_1_0_0(PsiBuilder builder_, int level_) {
    if (!recursion_guard_(builder_, level_, "unary_cond_1_0_0")) return false;
    boolean result_ = false;
    Marker marker_ = builder_.mark();
    result_ = consumeToken(builder_, PIG_IS);
    result_ = result_ && unary_cond_1_0_0_1(builder_, level_ + 1);
    result_ = result_ && consumeToken(builder_, PIG_NULL);
    if (!result_) {
      marker_.rollbackTo();
    }
  ...
  // (NOT)?
  private static boolean unary_cond_1_0_0_1(PsiBuilder builder_, int level_) {
    if (!recursion_guard_(builder_, level_, "unary_cond_1_0_0_1")) return false;
    consumeToken(builder_, PIG_NOT);
    return true;
  }
